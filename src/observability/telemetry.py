...
def initialize_telemetry(service_name: str = "complianceiq", otlp_endpoint: str = None) -> None:
    """
        ...
        async def wrapper(*args, **kwargs):
            ...
            with tracer.start_as_current_span(agent_name) as span:
                ...
                try:
                    ...
                    span.set_attribute("agent.duration_ms", int((time.time() - start_time) * 1000))
                    return result
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            with tracer.start_as_current_span(f"foundry_iq.{query_type}") as span:
                ...
                start_time = time.time()
                result = await func(*args, **kwargs)
                span.set_attribute("foundry_iq.duration_ms", int((time.time() - start_time) * 1000))
                if isinstance(result, dict) and "citations" in result:
                    span.set_attribute("foundry_iq.citations_returned", len(result.get("citations", [])))
                return result
        return wrapper
    return decorator
