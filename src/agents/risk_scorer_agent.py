...
EU_AI_ACT_ANNEX_III_HIGH_RISK = {
    "employment_hiring": {
        "keywords": ["cv", "resume", "hiring", "recruitment", "job application", 
                     "candidate", "employment", "workforce"],
        "article": "Annex III, point 4",
        "description": "AI used for recruitment or selection of natural persons"
    },
    "education_vocational": {
        "keywords": ["student", "exam", "assessment", "grading", "education",
                     "learning", "academic", "score", "test"],
        "article": "Annex III, point 3",
        "description": "AI used to determine access to educational institutions"
    },
    "biometric_identification": {
        "keywords": ["face", "facial", "fingerprint", "biometric", "iris",
    ...
    "critical_infrastructure": {
        "keywords": ["energy", "water", "gas", "electricity", "transport", 
                     "infrastructure", "safety component"],
        "article": "Annex III, point 2",
        "description": "AI as safety component of critical infrastructure"
    },
    "law_enforcement": {
        "keywords": ["police", "criminal", "suspect", "surveillance", 
                     "law enforcement", "court", "judicial"],
        "article": "Annex III, point 6",
        "description": "AI used by law enforcement"
    },
    "healthcare": {
        "keywords": ["medical", "health", "diagnosis", "patient", "clinical",
                     "disease", "treatment", "hospital", "doctor"],
        "article": "Annex III, point 5b",
        "description": "AI as medical device"
    },
    "credit_scoring": {
        "keywords": ["credit", "loan", "insurance", "financial", "bank",
                     "mortgage", "creditworthiness"],
        "article": "Annex III, point 5a",
        "description": "AI to evaluate creditworthiness"
    },
    "public_services": {
        "keywords": ["welfare", "benefits", "social security", "public service",
                     "government", "asylum", "border"],
...
class RiskScorerAgent:
    ...
    def _classify_risk_tier(self, system_profile: SystemProfile) -> tuple[EUAIActRiskTier, list[RiskFinding], list[str]]:
        """
        ...
        if not system_profile.human_oversight and system_profile.autonomy_level in ["full_autonomy", "high_autonomy"]:
            if is_high_risk:
                findings.append(RiskFinding(
    ...
    def _calculate_compliance_percentage(self, gap_matrix: GapMatrix) -> float:
        ...
        Returns 0.0-100.0
        """
        applicable_gaps = [g for g in gap_matrix.gaps if g.status != ComplianceStatus.NOT_APPLICABLE]
    ...
    async def score(self, gap_matrix: GapMatrix) -> RiskScorecard:
        ...
        critical_count = sum(1 for g in gap_matrix.gaps if g.severity == GapSeverity.CRITICAL and g.status == ComplianceStatus.NON_COMPLIANT)
        high_count = sum(1 for g in gap_matrix.gaps if g.severity == GapSeverity.HIGH and g.status == ComplianceStatus.NON_COMPLIANT)
        medium_count = sum(1 for g in gap_matrix.gaps if g.severity == GapSeverity.MEDIUM and g.status == ComplianceStatus.NON_COMPLIANT)
        low_count = sum(1 for g in gap_matrix.gaps if g.severity == GapSeverity.LOW and g.status == ComplianceStatus.NON_COMPLIANT)
