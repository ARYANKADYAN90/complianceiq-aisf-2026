...
from src.models.remediation_plan import RemediationPlan, RemediationItem, RemediationPriority
...
REMEDIATION_TEMPLATES = {
    "ART_09": {
        "default_actions": ["Establish continuous risk management system", "Document identified risks and mitigations"],
        "typical_owner_roles": "Compliance Officer",
        "typical_effort_days": 10,
        "article_reference": "Article 9"
    },
    "ART_10": {
        "default_actions": ["Implement bias testing protocols", "Document data provenance and governance"],
        "typical_owner_roles": "ML Engineer",
        "typical_effort_days": 7,
        "article_reference": "Article 10"
    },
    "ART_11": {
        "default_actions": ["Draft complete technical documentation prior to market placement"],
        "typical_owner_roles": "Technical Writer",
        "typical_effort_days": 15,
        "article_reference": "Article 11"
    },
    "ART_12": {
        "default_actions": ["Implement automated event logging mechanism", "Set up secure log retention"],
        "typical_owner_roles": "DevOps",
        "typical_effort_days": 5,
        "article_reference": "Article 12"
    },
    "ART_13": {
        "default_actions": ["Draft comprehensive instructions for users", "Add transparency disclosures"],
        "typical_owner_roles": "Product Manager",
        "typical_effort_days": 5,
        "article_reference": "Article 13"
    },
    "ART_14": {
        "default_actions": ["Implement human-in-the-loop override interface", "Halt fully automated critical decisions"],
        "typical_owner_roles": "Engineering Lead",
        "typical_effort_days": 8,
        "article_reference": "Article 14"
    },
    "ART_15": {
        "default_actions": ["Conduct robustness and adversarial testing", "Establish accuracy benchmarks"],
    ...
    "ART_20": {
        "default_actions": ["Develop post-market monitoring plan", "Create incident reporting dashboard"],
        "typical_owner_roles": "Product Manager",
        "typical_effort_days": 12,
        "article_reference": "Article 20"
    },
    "ANNEX_IV": {
        "default_actions": ["Provide detailed system architecture diagrams", "Document training methodology"],
        "typical_owner_roles": "Technical Writer",
        "typical_effort_days": 15,
...
class RemediationAgent:
    ...
    async def _generate_item_for_gap(self, gap: ComplianceGap, risk_tier: EUAIActRiskTier) -> list[RemediationItem]:
        """Generate remediation items for one gap using Foundry IQ."""
        template = REMEDIATION_TEMPLATES.get(gap.requirement_id, {
    ...
    async def plan(self, gap_matrix: GapMatrix, scorecard: RiskScorecard) -> RemediationPlan:
        ...
        actionable_gaps = [g for g in gap_matrix.gaps if g.status != ComplianceStatus.COMPLIANT]
    ...
    def format_roadmap_summary(self, plan: RemediationPlan) -> str:
        ...
        def _format_section(title: str, items: list) -> str:
            ...
            for item in items:
                lines.append(f"- [{item.owner_role}] {item.action_title} ({item.effort_days} days)")
            return "\n".join(lines)
