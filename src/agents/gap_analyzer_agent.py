...
import logging
from src.models.gap_matrix import GapMatrix, ComplianceGap, ComplianceStatus, GapSeverity
...
REQUIREMENTS_TO_CHECK = [
    {
        ...
        "evidence_needed": ["Risk register", "Risk management procedure", "Residual risk assessment"]
    },
    {
        "id": "ART_10",
        ...
        "evidence_needed": ["Data quality assessment", "Bias testing results", "Data governance policy"]
    },
    {
        "id": "ART_11",
        ...
        "evidence_needed": ["Annex IV documentation", "System card", "Architecture documentation"]
    },
    {
        "id": "ART_12",
        ...
        "evidence_needed": ["Audit logs", "Event logging system", "Log retention policy"]
    },
    {
        "id": "ART_13",
        ...
        "evidence_needed": ["User-facing documentation", "Instructions for use", "Capability limitations disclosure"]
    },
    {
        "id": "ART_14",
        ...
        "evidence_needed": ["Human review mechanism", "Override procedure", "Oversight protocol"]
    },
    {
        "id": "ART_15",
        ...
        "evidence_needed": ["Accuracy benchmarks", "Robustness testing", "Security assessment"]
    },
    {
        "id": "ART_17",
        ...
        "evidence_needed": ["QMS documentation", "Version control policy", "Change management procedure"]
    },
    {
        "id": "ART_20",
        ...
        "evidence_needed": ["Monitoring plan", "Incident reporting procedure", "Performance metrics dashboard"]
    },
    {
        "id": "ANNEX_IV",
        ...
        "evidence_needed": ["System description", "Design specifications", "Monitoring metrics", 
                            "Training methodology", "Validation results", "Post-deploy monitoring plan",
                            "Standards applied", "Conformity declaration"]
    }
]
...
class GapAnalyzerAgent:
    ...
    async def _check_single_requirement(self, requirement: dict, system_profile: SystemProfile) -> ComplianceGap:
        ...
        description_text = " ".join(result["gaps"]) if result["gaps"] else requirement["description"]
    ...
    async def analyze(self, system_profile: SystemProfile) -> GapMatrix:
        ...
        for i, gap in enumerate(gaps):
            if isinstance(gap, Exception):
                logger.error(f"Failed to check requirement {self.requirements[i]['id']}: {str(gap)}")
                # Create a fallback gap instead of crashing the pipeline
                valid_gaps.append(ComplianceGap(
    ...
    async def get_high_priority_gaps(self, gap_matrix: GapMatrix) -> list[ComplianceGap]:
        """Returns only CRITICAL and HIGH severity NON_COMPLIANT gaps, sorted by severity."""
        high_priority = [
            gap for gap in gap_matrix.gaps
            if gap.status == ComplianceStatus.NON_COMPLIANT and 
