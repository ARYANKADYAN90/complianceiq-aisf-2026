...
import asyncio
from src.mock.mock_data import MOCK_SYSTEM_PROFILE, MOCK_GAP_MATRIX, MOCK_RISK_SCORECARD, MOCK_REMEDIATION_PLAN, MOCK_REPORTS
...
class ComplianceIQOrchestrator:
    ...
    def __init__(self, mock_mode: bool = None):
        ...
        logger.info(f"ComplianceIQOrchestrator initialized. Mock mode: {self.mock_mode}")
    ...
    async def run(self, files: list, progress_callback: callable = None) -> dict:
        ...
        with tracer.start_as_current_span("Full Compliance Pipeline") as pipeline_span:
            ...
            try:
                ...
                try:
                    system_profile = await asyncio.wait_for(self.scanner.scan(files), timeout=30.0)
                    ...
                    update("🔍 Scanner Agent", "timeout ⚠️ (fallback applied)", system_profile)
                
                # Agent 2: Gap Analyzer
                update("🧠 Gap Analyzer (Foundry IQ)", "running")
                try:
                    gap_matrix = await asyncio.wait_for(self.gap_analyzer.analyze(system_profile), timeout=120.0)
                    update("🧠 Gap Analyzer (Foundry IQ)", f"complete ✅ — {gap_matrix.total_gaps} gaps found")
                except asyncio.TimeoutError:
                    logger.warning("Gap Analyzer Agent timed out. Using mock data.")
                    gap_matrix = GapMatrix(**MOCK_GAP_MATRIX)
                    update("🧠 Gap Analyzer (Foundry IQ)", "timeout ⚠️ (fallback applied)")
                
                # Agent 3: Risk Scorer
                update("⚖️ Risk Scorer", "running")
                try:
                    scorecard = await asyncio.wait_for(self.risk_scorer.score(gap_matrix), timeout=30.0)
                    update("⚖️ Risk Scorer", f"complete ✅ — {scorecard.risk_tier.value}")
                except asyncio.TimeoutError:
                ...
                try:
                    roadmap = await asyncio.wait_for(self.remediation_planner.plan(gap_matrix, scorecard), timeout=120.0)
                    update("🗺️ Remediation Planner (Foundry IQ)", f"complete ✅ — {len(roadmap.items)} actions")
                except asyncio.TimeoutError:
                    logger.warning("Remediation Planner timed out. Using mock data.")
                    roadmap = RemediationPlan(**MOCK_REMEDIATION_PLAN)
                    update("🗺️ Remediation Planner (Foundry IQ)", "timeout ⚠️ (fallback applied)")
                
                # Agent 5: Report Generator
                update("📝 Report Generator", "running")
                try:
                    reports = await asyncio.wait_for(self.report_generator.generate(
                ...
                try:
                    verified = await asyncio.wait_for(self.verifier.verify(reports, gap_matrix), timeout=120.0)
                    update("✔️ Verification Agent", 
                           "complete ✅" if verified.is_verified else "complete ⚠️ flags found")
                except asyncio.TimeoutError:
                ...
                pipeline_span.set_attribute("pipeline.risk_tier", scorecard.risk_tier.value)
                pipeline_span.set_attribute("pipeline.compliance_pct", scorecard.compliance_percentage)
