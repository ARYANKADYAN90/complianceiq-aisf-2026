...
class FoundryIQClient:
        ...
        async def _do_query():
            async with (
                ...
                return {
                    ...
                    "sources_used": (
                        result.sources_used if hasattr(result, "sources_used") else []
                    ),
    ...
    @retry_with_exponential_backoff(max_retries=3)
    async def check_requirement_compliance(
        self, requirement: str, system_profile: dict
    ) -> dict:
        ...
        if self.mock_mode:
            ...
            gap = next(
                (
                    g
                    for g in MOCK_GAP_MATRIX["gaps"]
                    if g["requirement_name"] == requirement
                ),
                None,
            )
            if gap:
    ...
    async def get_remediation_steps(self, gap_description: str, article: str) -> dict:
        ...
        if self.mock_mode:
            return {
                ...
                "citations": [
                    f"Foundry IQ: {article} mandates human intervention capability."
                ],
            }
    ...
    async def verify_claim(self, claim: str, cited_article: str) -> dict:
        ...
        is_verified = (
            "supported" in response["answer"].lower()
            and "contradicted" not in response["answer"].lower()
        )
        ...
        return {
            ...
            "flag": (
                None
                if is_verified
                else "Claim may not be fully supported by citations."
            ),
        }
