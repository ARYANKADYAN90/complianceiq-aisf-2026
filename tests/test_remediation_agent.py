...
from src.models.remediation_plan import RemediationPlan, RemediationPriority, RemediationItem
...
async def test_generate_item_priority_logic(agent):
    ...
    gap_critical = ComplianceGap(
        requirement_id="ART_14", requirement_name="Oversight", article_reference="Art 14",
        status=ComplianceStatus.NON_COMPLIANT, severity=GapSeverity.CRITICAL,
        description="test", missing_evidence=[], remediation_hint="", citations=[], confidence_score=0.9
    )
    ...
    gap_medium = ComplianceGap(
        requirement_id="ART_10", requirement_name="Data", article_reference="Art 10",
        status=ComplianceStatus.NON_COMPLIANT, severity=GapSeverity.MEDIUM,
        description="test", missing_evidence=[], remediation_hint="", citations=[], confidence_score=0.9
    )
...
async def test_plan_mock_mode():
    ...
    assert plan.immediate_actions[0].action_title == "Implement human oversight mechanism"
...
def test_format_roadmap_summary(agent):
    ...
    plan = RemediationPlan(
        items=[
            RemediationItem(
                item_id="REM-001", gap_reference="1", action_title="Urgent fix", 
                action_description="x", owner_role="CTO", effort_days=5, 
                priority=RemediationPriority.IMMEDIATE, article_reference="Art 1", 
                citation="", success_criteria="", dependencies=[]
            ),
            RemediationItem(
                item_id="REM-002", gap_reference="1", action_title="Later fix", 
                action_description="x", owner_role="Dev", effort_days=10, 
                priority=RemediationPriority.THIRTY_DAYS, article_reference="Art 1", 
                citation="", success_criteria="", dependencies=[]
            )
        ],
