...
with st.sidebar:
    ...
    st.markdown("---")
    st.link_button("⭐ Star on GitHub", "https://github.com/microsoft/agents-league-aisf")
...
with tab1:
    ...
    st.caption("Supported formats: PDF, DOCX, TXT, JSON (model cards). Max 10MB per file.")
    ...
    if mock_mode:
        st.info("📋 No files needed in mock mode. We'll analyze a sample 'TalentFilter Pro' CV screening AI system.")
    ...
    run_button = st.button("🚀 Run Compliance Analysis", 
                            type="primary", 
    ...
    if run_button:
        ...
        def progress_callback(agent_name, status, output):
            ...
            for key in agent_names:
                if agent_name.lower().replace("agent", "").strip() in key.lower() or agent_name in key:
                    matched_key = key
                    break
        ...
        with st.spinner("Analysis in progress..."):
            ...
            try:
                ...
                status_text.success("✅ Analysis complete! View results in the Results tab.")
                st.balloons()
...
with tab2:
    if "results" not in st.session_state:
        ...
        col1.metric("EU AI Act Risk Tier", risk_tier,
                    delta="⚠️ Strict Requirements Apply" if is_high else "",
                    delta_color="inverse" if is_high else "normal")
        col2.metric("Compliance Score", f"{scorecard['compliance_percentage']:.1f}%",
                    delta=f"-{scorecard['critical_count']} critical gaps" if scorecard['critical_count'] > 0 else "Compliant",
                    delta_color="inverse")
        
        total_gaps = len(gap_matrix["gaps"])
        found_gaps = scorecard["critical_count"] + scorecard["high_count"] + scorecard["medium_count"] + scorecard["low_count"]
        col3.metric("Gaps Identified", found_gaps,
        ...
        for gap in gap_matrix["gaps"]:
            ...
            with st.expander(f"{color_icon} {gap['requirement_name']} — {status}"):
                ...
                st.write(f"**Missing Evidence**: {', '.join(gap['missing_evidence']) if gap['missing_evidence'] else 'None'}")
        ...
        def display_items(col, title, priority_value):
            items = [item for item in roadmap["items"] if item["priority"] == priority_value]
            col.markdown(f"**{title}** ({len(items)})")
            for item in items:
                col.info(f"**{item['action_title']}**\n\n"
                         f"👤 {item['owner_role']}\n\n"
...
with tab3:
    if "results" not in st.session_state:
        ...
        col1.download_button("📊 Executive Report", 
                             reports["executive_summary"].encode('utf-8'),
                             "executive_report.md", mime="text/markdown")
        col2.download_button("🔧 Technical Report",
                             reports["technical_findings"].encode('utf-8'),
                             "technical_report.md", mime="text/markdown")
        col3.download_button("📋 Compliance Certificate",
                             reports["certificate_draft"].encode('utf-8'),
                             "compliance_certificate.md", mime="text/markdown")
